...
Next.js 16 ships version-matched documentation at `node_modules/next/dist/docs/`. It is the source of truth for this version's APIs and its deprecation notices are binding — framework code here is written against it (see `AGENTS.md`).
...
The suite includes **hygiene tests that assert on the repo itself** — `dockerfile-hygiene`, `lockfile-hygiene`, `manifest-hygiene`, `fetch-host-hygiene`, `version-hygiene`, `revalidation-hygiene`, `backup-hygiene`, `git-env-hygiene`, `a11y-class-hygiene`, `override-hygiene`, `regexp-source-hygiene`, `renovate-hygiene`, `braindump-to-task-hygiene`, `focus-sound-migration-hygiene`, `inbox-write-hygiene`, `write-notice-hygiene`, `empty-upsert-hygiene`, `source-encoding-hygiene`, `integration-docblock-hygiene`, `env-drift`, `ci-docs-only`, `ci-job-deps`, `ci-interruptible`, `log-retention`, `capture-orphan-window`, `enum-constraint-sync`. If one fails, the repo drifted; fix the drift, don't relax the test. The list is **curated, not mechanical** — `workspace-resolver-hygiene` is a pure helper for ordinary unit tests rather than a check that reads the tree, so it is deliberately absent, which is also why nothing enforces this list's completeness. **Six are compensating controls rather than style, and relaxing any of them reopens what it replaced:** `fetch-host-hygiene` stands in for a demoted SAST rule (#83), so weakening it reopens a CWE-918 hole; `regexp-source-hygiene` stands in for a second demoted rule (#234, CWE-185) and asserts that every `new RegExp` builds its pattern from a file-level literal constant, an `escapeForRegExp` call or a test fixture — it also asserts it can *see* every file that rule reports on, because its first version silently missed four of them and called them clean; `a11y-class-hygiene` is the **only** check in the repo that can see a **killed focus indicator** — an element that removes the UA outline and replaces it with nothing, or with a colour swap — or a **state-dependent contrast failure**; axe implements neither, having no rule for any focus criterion at all — not 2.4.7, not 2.4.11, not 2.4.13 — and measuring only what is painted during the scan. Every site #109 and #117 list shipped green because nothing else could look, and the gate then found three more they had both missed. What it enforces is **2.4.7 Focus Visible (AA)** plus a house rule reaching toward **2.4.13 Focus Appearance (AAA)**; it does **not** cover 2.4.11 Focus Not Obscured, which is AA and which nothing here checks — that gap is stated in the module's own docblock, and #258 is why this sentence no longer claims it. `write-notice-hygiene` is the only one that can see a *missing* failure message or an unannounced wait, neither of which is a rule axe has and both of which shipped green four times (#210, #218, #225, #246) because every check in the suite was per-file; `override-hygiene` keeps a `package.json` override from drifting out from under the Renovate rule that pins it inside a patched major (#161). `capture-orphan-window` is the only thing that can see the offline capture queue's orphan window drift away from `GUEST_SANDBOX_TTL_HOURS` — two surfaces stating one fact, where the client cannot read the server's variable and `env-drift` compares key sets rather than values, so a 24 becoming a 72 passes it; the number is `/privacy`'s third retention trigger for `df-capture-queue`, so a drift makes a legal page false rather than untidy, and CI can only compare the two *defaults* (#175). `renovate-hygiene` guards four properties of `.gitlab/renovate.json`, three of them from #243, and nothing in this repo's CI runs `renovate-config-validator` — so all four would otherwise go unchecked here. **Three of them the validator cannot catch even when it is run**, and those are the ones it exists for: no concurrency limit or schedule under `vulnerabilityAlerts`, because Renovate ignores all of them for a security PR and the validator accepts the key regardless; a `logLevelRemap` entry promoting `Automerge on PR creation failed` to `warn`, because Renovate swallows that failure at `debug` — five unarmed MRs consequently produced no output anywhere — and a `newLogLevel` outside the enum validates clean; and any per-package `automerge: false` entry sitting **below** the blanket `automerge: true` rule, because `packageRules` is an **ordered** list whose later entries win, so a deny entry written above that rule is inert while still reading as though the exception were in force — measured on renovate 43, both orderings return `Config validated successfully` at exit 0. ⚠️ That ordering guard's real-config arm is **deliberately vacuous today**: the config has no per-package deny entry, so it iterates an empty list, and its synthetic cases carry the coverage while two further arms read the real file — one asserting the blanket rule still resolves, the other that every automerge-setting rule's `matchPackageNames` is a literal name, because Renovate resolves that key through `matchRegexOrGlobList` (a bare `*`, a minimatch glob, a `/…/` regex, a leading `!`) and this module reads literal names only; a pattern entry made the old helper report a denied package as denied while Renovate automerged it. It was written before the shape exists because whoever adds the first deny entry is the person who cannot see the trap; its docblock says all of this. **The fourth the validator does reject on its own**: a `schedule` window whose cron minute is not `*`. It is guarded here anyway because that window is the only thing stopping the frequent automerge-recovery schedule from opening update MRs of its own, and a check nothing runs is not a check — but it is the same check moved, not a gap filled, and `renovate-hygiene.ts` says so at the assertion.
...
1. **Bump all three version values to the same number** — `package.json` `version`, and `charts/dlectroflow/Chart.yaml`'s `version` **and** `appVersion` (they're lockstep by decision; the reasoning is in `Chart.yaml`). Bump `package-lock.json`'s two `version` fields (root and `packages.""`) to match — `version-hygiene` does **not** check the lockfile and nothing else does either, so it has drifted more often than not. Measured across the tags: it lagged `package.json` by one release at **v0.2.0** (`0.1.0`) and **v0.3.0** (`0.2.0`); at **v0.4.0** it read `0.3.0`, but so did `package.json` — that is #148 below, the whole tree, not a lockfile-specific miss; and **v0.5.0** was the first cut where it carried the release version. Unchecked is the problem, not a reliable direction of error.
