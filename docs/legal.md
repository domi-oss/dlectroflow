...
| **Guest TTL** (`GUEST_SANDBOX_TTL_HOURS`, default 24) | "about a day" in three places, the `df_guest` cookie lifetime, **and the offline capture queue's orphan window** — `CAPTURE_ORPHAN_WINDOW_HOURS` (`src/lib/capture-queue.ts`) is a *client* constant stating the same fact, because the browser cannot read a server variable and this repo has no `NEXT_PUBLIC_*` at all. It is the page's **third** retention trigger for `df-capture-queue`, *"until it can no longer be saved to any account you can reach"*, so a drift makes a retention promise false rather than merely untidy. `src/lib/capture-orphan-window.test.ts` asserts `client >= server` on the two **defaults** — and cannot see a deployment that overrides the variable, so a self-host running a longer guest TTL must raise the client constant to match (#175) | `src/lib/purge.ts` → `guestSandboxTtlHours`, `src/lib/capture-queue.ts` → `CAPTURE_ORPHAN_WINDOW_HOURS`; Privacy → *Cookies*, *How long I keep it*; Terms → *Who can use it* |
