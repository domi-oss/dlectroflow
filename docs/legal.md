...
|---|---|
| Self-service data export (#129 — still no `src/app/api/account/` directory; the `/api/account/` entry in `AUTHENTICATED_PREFIXES` reserves the prefix, it does not implement a route) | Access and portability are handled **by hand** from the contact address, within the statutory one month. The Terms say nothing about export at all, and deliberately link to nothing: **when #129 ships, the one sentence to add is at the end of Terms → *Your data* → *What the backups can and cannot do***, where a source comment marks the slot. `src/app/terms/page.test.tsx` fails if a link matching `/export/i` or a "you can export/download your data" claim appears before then |
