...
| Google singleton migration | Fresh start means no token migration. **The drop happens in Phase A**, together with `OWNER_WORKSPACE_ID`, rather than waiting for Phase C — leaving the singleton alive while its only guard has been deleted is worse than removing it. The owner reconnects once, after the Phase A deploy. **Correction (#119): what Phase A dropped was the database *default* and the existing rows, not the shared row.** `src/lib/google.ts` still supplies `id: SINGLETON_ID` explicitly on read and write, so one instance-wide row is re-created on demand and sync never went down — and the singleton's only remaining guard between Phase A and Phase C is the handler-level owner check on the two OAuth routes, which #119 adds. |
