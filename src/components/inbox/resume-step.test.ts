...
describe("firstResumableStep", () => {
  it("returns a resumable, not-done step", () => {
    const items = [
      item("i1", [
        step({ id: "s1", text: "book flight", resumable: true, done: false }),
      ]),
    ];
    expect(firstResumableStep(items)).toEqual({
      id: "s1",
      text: "book flight",
    });
  });
  ...
  it("returns null when there are no resumable steps", () => {
    const items = [
      item("i1", [
        step({ id: "s1", text: "book flight", resumable: false, done: false }),
      ]),
      item("i2", [
        step({ id: "s2", text: "pack bag", resumable: false, done: true }),
      ]),
    ];
    expect(firstResumableStep(items)).toBeNull();
  });
  ...
  it("picks the first eligible step across multiple items (createdAt-desc order)", () => {
    const items = [
      item("newest", [
        step({ id: "s1", text: "not eligible", resumable: false, done: false }),
      ]),
      item("older", [
        step({ id: "s2", text: "eligible", resumable: true, done: false }),
      ]),
      item("oldest", [
        step({ id: "s3", text: "also eligible", resumable: true, done: false }),
      ]),
    ];
