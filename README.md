...
| ⏱️ Focus timer — true pause/resume, one-number setup screen | ✅ works — and finishing a step no longer dead-ends: a 5-second countdown opens the **next step's start screen** (nothing starts its timer), with **Escape** and a **Stay here** button as escapes. **Hyper focus mode** extends the same chaining to single-task to-dos and is **off by default**. |
